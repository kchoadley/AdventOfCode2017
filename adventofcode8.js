...
function part1(input) {
    ...
    input.forEach(element => {
        ...
        out_reg = parsed.shift();
        op = parsed.shift();
        amount = parseInt(parsed.shift());
        parsed.shift();       // discard if
        comp_r = parsed.shift();
        cond = parsed.shift();
        comp_n = parseInt(parsed.shift());

        if(registers.has(out_reg)) {
            out_reg_val = registers.get(out_reg);
        }
        if(registers.has(comp_r)) {
            comp_r_val = registers.get(comp_r);
        }

        if(cond.includes("!")) {
            if(comp_r_val != comp_n) {
                if(op == "inc")
                    out_reg_val += amount;
                else
                out_reg_val -= amount;
            ...
            if(comp_r_val == comp_n) {
                console.log(comp_r_val, cond, comp_n);
                if(op == "inc")
                    out_reg_val += amount;
                else
                    out_reg_val -= amount;
        ...
        if(out_reg_val > solution2) { solution2 = out_reg_val; }
        registers.set(out_reg, out_reg_val);
    });

    let largest = 0;
    let iterator = registers.entries();

    let arr = Array.from(registers.entries());

    arr.forEach(element => {
        if(element[1] > solution) { solution = element[1] }
    })
