"use strict"
let input = `mmqyju (156) -> rjzvwv, noybkx
...
function Node(name, weight, children){
    this.name = name;
    this.weight = weight;
...
function Tree(input) {
    ...
    input.forEach(element => {  // initialize tree collection
        let regex = /([a-z]){1,}/g;
        let parsed = element.match(regex);
        let children = [];
        let name = "";
        while(parsed.length>1) {
            children.push(parsed.pop());
        }
        name = parsed.pop();
        regex = /([0-9]){1,}/;
        let weight = element.match(regex);
    ...
    this.get_children = function(node) {
        ...
        for(let i = 0; i < node.children.length; i ++) {
            let temp_node = this.collection.filter((n) => n.name == node.children[i]);
    ...
    this.get_root = function() {
        let root = this.collection.filter((node) => node.name == "svugo");
        return root[0];
    }
    
    this.set_total_weight = function(node) {   // determine total weight of each node
        node.total_weight = node.weight;
        let children = this.get_children(node);
        children.forEach(child => {
            node.total_weight += this.set_total_weight(child);
        })
        return node.total_weight;
    }

    this.bad_node_finder = function(bad_child, correct_weight) {
        let solution = bad_child.weight + correct_weight - bad_child.total_weight;
        let children = this.get_children(bad_child);
        for(let i = 1; i < children.length; i++) {
            if(children[i].total_weight != children[i-1].total_weight) {  // find the unbalanced disk
                let correct_weight = 0;
                let bad_child;
                for(let i = 1; i < children.length; i++) {      // determine correct weight
                    if(children[i].total_weight == children[i-1].total_weight) {
                        correct_weight = children[i].total_weight;
                        break;
                    }
                }
                for(let i = 0; i < children.length; i++) {      // determine incorrectly weighted program
                    if(children[i].total_weight != correct_weight) {
                        bad_child = children[i];
                        break;
                    }
                }
                solution = this.bad_node_finder(bad_child, correct_weight);
            }
        }
        return solution;
    }

    this.get_correct_weight = function() {
        let solution = 0;
        this.set_total_weight(this.get_root());
        this.collection.forEach(element => {
            if(element.children.length>1) {
                let children = this.get_children(element);
                for(let i = 1; i < children.length; i++) {
                    if(children[i].total_weight != children[i-1].total_weight) {  // find the unbalanced disk
                        let correct_weight = 0;
                        let bad_child;
...
function part1(input) {
    ...
    input.forEach(element => {
        let regex = /([a-z]){1,}/g;
        let parsed = element.match(regex);  // get all nodes (names)
        while(parsed.length>1) {
            child.add(parsed.pop());
        }
        parent.add(parsed.pop());
    });
    parent.forEach( element => {
        if(!child.has(element))     // if element is not a child
            solution = element;
    })
    return solution;
}
...
function part2(input) {
    ...
    return tree.get_correct_weight();
}
